...
# This script was adapted in 2024 by UNHOLSTER S.A. from the orginal located at  
# https://github.com/NVIDIA/NeMo/blob/main/scripts/speaker_tasks/filelist_to_manifest.py
# 
# The changes were made for the user to be able to customize the creation of
# segments. This was made by:
#
#   - creating and initializing new global variables `SEGMENTS_DIR` and 
#     `segment_duration`;
#   - deleting the global variable `CWD` to allow the user to save the segments
#     in a directory of their choice, not only in the current working directory;
#   - modifying the function `filter_manifest_line` to incorporate the global
#     variables `DURATIONS` and `SEGMENTS_DIR` in its scope;
#   - modifying the function `main` to incorporate the new args `--segment_duration`
#     and `--segments_dir` and to include the global variables `DURATIONS` 
#     and `SEGMENTS_DIR` in its scope, in order to modify their values read by
#     the function `filter_manifest_line`.
...
Original Args (2020, NVIDIA CORPORATION):
...
Added Args (2024, UNHOLSTER S.A.):
--segment_duration: Duration of audio segments to create an augmented dataset.
--segments_dir: Path to the directory where the created segments will be saved.
"""
...
MIN_ENERGY = 0.01
SEGMENTS_DIR = os.getcwd()  # Added (2024, UNHOLSTER S.A.)
segment_duration = 3    # Added (2024, UNHOLSTER S.A.)


def filter_manifest_line(manifest_line):
    global DURATIONS    # Added (2024, UNHOLSTER S.A.)
    global SEGMENTS_DIR    # Added (2024, UNHOLSTER S.A.)
    split_manifest = []
    ...
    to_path = os.path.join(SEGMENTS_DIR, label)
...
def main(filelist, manifest, id, out, split=False, create_segments=False, 
         segment_duration=3, segments_dir=None, min_count=10):  # Modified by UNHOLSTER S.A., 2024
    ...
    if create_segments:
        global DURATIONS    # Added by UNHOLSTER S.A., 2024
        global SEGMENTS_DIR    # Added by UNHOLSTER S.A., 2024
        DURATIONS = sorted([segment_duration], reverse=True)  # Added by UNHOLSTER S.A., 2024
        SEGMENTS_DIR = segments_dir  # Added by UNHOLSTER S.A., 2024
        print(f"creating and writing segments to {SEGMENTS_DIR}")  # Added by UNHOLSTER S.A., 2024
